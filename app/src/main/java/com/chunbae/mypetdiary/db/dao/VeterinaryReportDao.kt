...
interface VeterinaryReportDao {
    ...
    // 처방약물목록 조회
    @Query("select * from vet_medicine where vet_id in (:vetId )")
    fun selectVMedicineList(vetId: List<Long?>?): List<VMedicine>?

    // 이미지 목록 조회
    @Query("select * from vet_images where vet_id in (:vetId )")
    fun selectVImageList(vetId: List<Long?>?): List<Images>?
}